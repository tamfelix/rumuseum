...
<nav class="flex inline-flex justify-between w-full">
    <div class="jumbotron jumbotron-fluid p-3 pl-3">
        ...
        <p class=" mb-4 ">
            <a href="javascript:logging()">{{ __('sections.slavistics') }}</a>
        </p>
    </div>
...
{{--MAIN SECTION--}}
    ...
    <footer class="footer-section   text-sm pl-4 text-gray-500 h-72">
        ...
        <div class=" col-span-4 h-36 mx-auto pt-10">
            @foreach($menuitems as $item)
                <a class="col-span-1 font-semibold p-4" href="{{ env('APP_URL').'/'. $item['link'] }}">
                    {{$item['title_ru']}}</a>
            @endforeach
        </div>
{{--COPYRIGHT--}}
        <section class="flex inline-flex w-full justify-between">
            <div class="my-auto">&copy; 1831-2022 {{ __('sections.museum') }} </div>
